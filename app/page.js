...
export default function ReconstructionTaxCalculator() {
  ...
  const lbl = { display: 'block', fontSize: '14px', fontWeight: 500, color: 'rgb(203,213,225)', marginBottom: '8px' };
  const card = {
  ...
  const grid3 = { display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr 1fr', gap: isMobile ? '16px' : '24px' };
      ...
      <div style={{ maxWidth: '960px', margin: '0 auto' }}>
        ...
        <div style={{ marginBottom: '48px' }}>
          <div style={{ display: 'flex', flexDirection: isMobile ? 'column' : 'row', alignItems: isMobile ? 'flex-start' : 'center', justifyContent: 'space-between', gap: '16px', marginBottom: '24px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
              <AlertTriangle style={{ width: '32px', height: '32px', color: 'rgb(239,68,68)', flexShrink: 0 }} />
              <h1 style={{ fontSize: isMobile ? '26px' : '36px', fontWeight: 700, fontFamily: 'Georgia, serif', letterSpacing: '0.02em', margin: 0 }}>Reconstruction Tax Calculator</h1>
            </div>
            <div style={{ textAlign: isMobile ? 'left' : 'right' }}>
              <div style={{ fontSize: '20px', fontWeight: 700, letterSpacing: '0.15em' }}>YAVARDI</div>
              <div style={{ fontSize: '12px', color: 'rgb(148,163,184)' }}>Technology Governance for Legal Operations</div>
            </div>
          </div>
          <p style={{ fontSize: isMobile ? '15px' : '18px', color: 'rgb(203,213,225)', margin: 0 }}>Calculate the hidden tax your firm pays for missing digital proof of authority</p>
        </div>
        ...
        <div style={card}>
          ...
          <div style={{ ...grid2, marginBottom: '24px' }}>
            ...
            <div>
              <label style={lbl}>Target Billable Hours / Attorney <span style={{ fontSize: '12px', color: 'rgb(100,116,139)', fontWeight: 400 }}>— Industry avg: 730</span></label>
          ...
          <div>
            <label style={lbl}>Realization Leakage Rate (%) <span style={{ fontSize: '12px', color: 'rgb(100,116,139)', fontWeight: 400 }}>— Industry avg: 12%</span></label>
        ...
        {results && !leadSubmitted && (
          <div style={{ ...card, border: '1px solid rgba(59,130,246,0.4)' }}>
            ...
            <button onClick={handleLeadSubmit} disabled={leadSubmitting} style={{ width: '100%', background: leadSubmitting ? 'rgb(30,58,138)' : 'rgb(37,99,235)', color: 'white', border: 'none', borderRadius: '8px', padding: '16px 24px', fontSize: '18px', fontFamily: 'Georgia, serif', fontWeight: 600, cursor: leadSubmitting ? 'default' : 'pointer' }}>
        ...
        {results && leadSubmitted && (
          <>
            {/* Hero */}
            <div style={{ background: 'linear-gradient(135deg, rgba(127,29,29,0.7), rgba(100,10,10,0.8))', borderRadius: '12px', padding: isMobile ? '24px 20px' : '40px 48px', marginBottom: '24px', border: '2px solid rgba(239,68,68,0.6)' }}>
              ...
              <div style={{ marginBottom: '36px' }}>
                ...
                <div style={{ fontSize: isMobile ? '52px' : '72px', fontWeight: 700, color: 'rgb(239,68,68)', fontFamily: 'Georgia, serif', lineHeight: 1 }}>{fmt(results.addrR)}</div>
                <div style={{ fontSize: '13px', color: 'rgb(185,130,130)', marginTop: '8px' }}>per year in write-downs tied directly to missing proof of authority</div>
              </div>
              <div style={grid3}>
                <div style={{ background: 'rgba(0,0,0,0.35)', borderRadius: '8px', padding: '16px 20px' }}>
                  ...
                  <div style={{ fontSize: '28px', fontWeight: 700, color: 'rgb(239,68,68)', fontFamily: 'Georgia, serif' }}>{fmt(results.addrR / 12)}</div>
                ...
                <div style={{ background: 'rgba(0,0,0,0.35)', borderRadius: '8px', padding: '16px 20px' }}>
                  ...
                  <div style={{ fontSize: '28px', fontWeight: 700, color: 'rgb(203,213,225)', fontFamily: 'Georgia, serif' }}>{fmt(results.realizationLeak)}</div>
                ...
                <div style={{ background: 'rgba(0,0,0,0.35)', borderRadius: '8px', padding: '16px 20px' }}>
                  ...
                  <div style={{ fontSize: '28px', fontWeight: 700, color: 'rgb(203,213,225)', fontFamily: 'Georgia, serif' }}>{fmt(results.firmWideRecorded)}</div>
            ...
            <div style={{ background: 'rgba(20,25,35,0.7)', borderRadius: '12px', padding: isMobile ? '24px 20px' : '28px 40px', marginBottom: '24px', border: '1px solid rgb(40,50,65)' }}>
              ...
              <div style={grid3}>
                <div>
                  ...
                  <div style={{ fontSize: isMobile ? '28px' : '36px', fontWeight: 700, color: 'rgb(185,50,50)', fontFamily: 'Georgia, serif' }}>{fmt(results.total)}</div>
                ...
                <div>
                  ...
                  <div style={{ fontSize: isMobile ? '28px' : '36px', fontWeight: 700, color: 'rgb(185,50,50)', fontFamily: 'Georgia, serif' }}>{fmt(results.monthly)}</div>
                ...
                <div>
                  ...
                  <div style={{ fontSize: isMobile ? '28px' : '36px', fontWeight: 700, color: 'rgb(185,50,50)', fontFamily: 'Georgia, serif' }}>{fmt(results.capacityLeak)}</div>
            ...
            {/* CTA */}
            <div style={{ background: 'linear-gradient(135deg, rgba(29,78,216,0.3), rgba(30,64,175,0.3))', borderRadius: '12px', padding: isMobile ? '24px 20px' : '32px 40px', border: '2px solid rgba(59,130,246,0.5)' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '16px', marginBottom: '20px' }}>
                <Calendar style={{ width: '40px', height: '40px', color: 'rgb(96,165,250)', flexShrink: 0 }} />
                <div>
                  <h2 style={{ fontSize: isMobile ? '22px' : '28px', fontWeight: 700, fontFamily: 'Georgia, serif', margin: 0 }}>Governance Discovery Call</h2>
              ...
              <p style={{ fontSize: '14px', color: 'rgb(100,116,139)', marginBottom: '28px', lineHeight: 1.7 }}>
                Your firm has an estimated <strong style={{ color: 'rgb(239,68,68)' }}>{fmt(results.addrR)}</strong> in annual write-downs tied directly to missing proof of authority. This call maps the specific authority failure points driving that exposure and what it takes to close them.
              </p>
              <a href={BOOKING_URL} target="_blank" rel="noopener noreferrer" style={{ display: 'block', width: '100%', background: 'rgb(37,99,235)', color: 'white', borderRadius: '8px', padding: '18px 24px', fontSize: '18px', fontFamily: 'Georgia, serif', fontWeight: 600, textAlign: 'center', textDecoration: 'none', boxSizing: 'border-box' }}>
